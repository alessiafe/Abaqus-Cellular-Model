# Y_RVE_Class.py
# This script implements a class for 2- and 3-dimensional Y-shaped RVE in Abaqus.
...
class Y_RVE_Class:
    ...
    def setup_BC(self, modelName, stepName, strains=[], moisture=0.12):
    ...
    def calculate_stiffness_matrix(self, umatpath='', sim=False, value=0.01, moisture=0.12):
    # Calculate the stiffness matrix of the RVE after applying 4 elementary strain cases.
    # Args:
    # - jobName = name of the job
    # - stepName = name of the step
    # - inc = increment number
    # Return:
    # - stiffnessMatrix = sitffness matrix of RVE

        nlgeom = ON
        numElCases = 9
        strains_mat = [[0.]*6 for i in range(numElCases)]
        stresses_mat = [[0.]*6 for i in range(numElCases)]

        # Set strain elementary cases
        eps_x = [value, 0., 0., 0., 0., 0., value, value, 0.]
        eps_y = [0., value, 0., 0., 0., 0., value, 0., value]
        eps_z = [0., 0., value, 0., 0., 0., 0., value, value]
        eps_yz = [0., 0., 0., value, 0., 0., 0., 0., 0.]
        eps_xz = [0., 0., 0., 0., value, 0., 0., 0., 0.]
        eps_xy = [0., 0., 0., 0., 0., value, 0., 0., 0.]

        if not(sim):
            # Create a model for each elementary case
            ...
            for k in range(numElCases):
                ...
                # Create step
                self.create_step(modelName=modelName, stepName=stepName, nlgeom=nlgeom,
                    previous='Initial', maxNumInc=100, initialInc=0.1, minInc=1e-5, maxInc=1)
                # Create boundary conditions
                self.setup_BC(modelName=modelName, stepName=stepName,
                    strains=[eps_x[k], eps_y[k], eps_z[k], eps_yz[k], eps_xz[k], eps_xy[k]], moisture=moisture)
                # Submit job
                self.submit_job(jobName, modelName, umatpath)
                # Calculate stresses and strains
                stresses_mat[k], strains_mat[k] = self.calculate_stress_strain(jobName, stepName, -1)
        else:
            # Set first model as reference for geometry and RPs names
            k = 0
            modelName = 'Model-{}'.format(k+1)
            stepName = 'Loading-{}'.format(k+1)
            # Create step
            ...
            for k in range(numElCases):
                ...
                print(stresses_mat[k], strains_mat[k])

        # Calculate stiffness matrix
        #print(stresses_mat)
        #print(strains_mat)
        self.stiffnessMatrix = [[0.]*6 for i in range(6)]
        # Calculate diagonal elements
        for i in range(6):
            self.stiffnessMatrix[i][i] = stresses_mat[i][i]/strains_mat[i][i]
        # Calculate out-of-diagonal elements
        k = [6, 7, 8]
        i = [0, 0, 1]
        j = [1, 2, 2]
        for n in range(3):
            stress = stresses_mat[k[n]]
            strain = strains_mat[k[n]]
            self.stiffnessMatrix[i[n]][j[n]] = (stress[i[n]]-self.stiffnessMatrix[i[n]][i[n]]*strain[i[n]])/strain[j[n]]
            self.stiffnessMatrix[j[n]][i[n]] = (stress[j[n]]-self.stiffnessMatrix[j[n]][j[n]]*strain[j[n]])/strain[i[n]]
        
        return self.stiffnessMatrix
    # end: def calculateStiffnessMatrix
