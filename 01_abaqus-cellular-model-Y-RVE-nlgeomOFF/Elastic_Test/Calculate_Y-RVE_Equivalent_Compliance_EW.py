...
################# SET PATHS #################
mainpath = sys.argv[10] # get working directory
os.chdir(mainpath) # set working path
modulespath = os.path.join(mainpath, 'Modules') # path to functions folder
sys.path.append(modulespath) # add folder path
umatpath = os.path.join(modulespath, 'LayersMatModel.f') # umat path to umat file
geompath = os.path.join(modulespath, 'growth_ring_data.csv') # path to geometry data file
...
def write_result_to_csv(result, filename):
    ...
    # Create dictionary to hold all columns as lists
    data_columns = {}
    max_rows = 0
    for key in headers:
        # Retrieve value
        val = result.get(key, [])
        # If value isn't a list, convert it to a single-item list
...
load = [2, 1.5, 70, 8, 10, 4] # load for each elementary case [mN]
