# Simulation of viscoelastic creep of Y-RVE tissues and growth ring

This folder contains all scripts and files used for the **simulation of viscoelastic creep** of the **Y-RVE tissues** (EW, TW, LW), as well as the **growth ring (GR)** composed of them. All simulations are conducted with non-linear geometry deactivated (**nlgeom = OFF**). The elastic properties must be known (calculated in`Elastic_Test`).
...
- **`run_for_MatSuMoTo.py`**  
  ...
  2. **`fit_prony_y-rve.py`**  
     Fits the Y-RVE creep compliances using a Kelvin–Voigt (Prony series) model with a fixed number of elements (4) and fixed characteristic times (0.1, 1, 10, 100). Then, the **gamma ratios** are computed as the ratio between the elastic compliance and the compliance of each KV element for each direction; these are then averaged (as the values are nearly identical across directions). The resulting gamma values are stored in a `.csv` file (new values are appended; the last row is always used) and written into the include file **`tissues_gamma_values.inc`** for the growth ring Fortran material model.
...
- **`Modules`**  
  ...
  - **`Y_RVE_Class.py`**  
  Python class defining the Y-RVE geometry (multi-layer cell wall model), boundary conditions, loading cases, and data extraction routines.
  ...
  - **`LayersMatModel.f`**  
  Fortran material model implementing the constitutive behaviour of the multilayer cell-wall model used for the Y-RVE tissues.

  - **`eng_const.inc`**  
  Include file storing the equivalent engineering constants of the tissues for the GR-RVE simulations.
  ...
  Include file storing the gamma values for the tissues (identical set for all tissues) used in the growth ring simulations.

  - **`layers_gamma_values.inc`**  
  Include file storing the gamma values for the cell-wall layers (identical set for all layers) used in the Y-RVE simulations.

  - **`layers_gamma_initial_values.csv`**  
  Stores initial gamma values of the cell-wall layers. Used when the surrogate model is running to optimize these values.
